...
describe("circumcircle", () => {
  it("finds the unit circle from three points on it", () => {
    expect(circumcircle({ x: 1, y: 0 }, { x: 0, y: 1 }, { x: -1, y: 0 })).toEqual({
      center: { x: 0, y: 0 },
      radius: 1,
    });
  });
  ...
  it("handles a right triangle (circumcenter is midpoint of hypotenuse)", () => {
    ...
    expect(circumcircle({ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 0, y: 3 })).toEqual({
      center: { x: 2, y: 1.5 },
      radius: 2.5,
    });
  });
  ...
  it("handles an equilateral triangle", () => {
    ...
    expect(result.center.y).toBeCloseTo(sqrt3 / 3);
    expect(result.radius).toBeCloseTo((2 * sqrt3) / 3);
  });

  it("handles a circle not centred at the origin", () => {
    // circle centred at (3, 4) with radius 5: use points (8,4), (3,9), (-2,4)
    expect(circumcircle({ x: 8, y: 4 }, { x: 3, y: 9 }, { x: -2, y: 4 })).toEqual({
      center: { x: 3, y: 4 },
      radius: 5,
    });
  });
