...
describe("circumcircle", () => {
  it("finds the unit circle from three points on it", () => {
    expect(circumcircle({ x: 1, y: 0 }, { x: 0, y: 1 }, { x: -1, y: 0 })._unsafeUnwrap()).toEqual({
      center: { x: 0, y: 0 },
      radius: 1,
    });
  });
  ...
  it("handles a right triangle (circumcenter is midpoint of hypotenuse)", () => {
    ...
    expect(circumcircle({ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 0, y: 3 })._unsafeUnwrap()).toEqual({
      center: { x: 2, y: 1.5 },
      radius: 2.5,
    });
  });
  ...
  it("handles an equilateral triangle", () => {
    ...
    const result = circumcircle({ x: 0, y: 0 }, { x: 2, y: 0 }, { x: 1, y: sqrt3 });
    assert(result.isOk());
  ...
  it("handles a circle not centred at the origin", () => {
    ...
    expect(circumcircle({ x: 8, y: 4 }, { x: 3, y: 9 }, { x: -2, y: 4 })._unsafeUnwrap()).toEqual({
      center: { x: 3, y: 4 },
      radius: 5,
    });
  });
  ...
  it("returns Err when the three points are collinear", () => {
    expect(circumcircle({ x: 0, y: 0 }, { x: 1, y: 1 }, { x: 2, y: 2 })._unsafeUnwrapErr()).toBe("collinear");
  });
});
